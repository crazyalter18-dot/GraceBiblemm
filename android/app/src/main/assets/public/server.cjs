...
${history ? history.map((h) => `${h.sender === "user" ? "User" : "Grace"}: ${h.text}`).join("\n") : ""}
User asks: ${message}
  ...
  try {
    ...
    try {
      ...
      if (myRes && enRes && Array.isArray(myRes.verses) && Array.isArray(enRes.verses)) {
        ...
        const verseNumbersSet = /* @__PURE__ */ new Set();
        myVerses.forEach((v) => {
        ...
        for (const vNum of sortedVerseNumbers) {
          const myV = myVerses.find((v) => parseInt(v.verse) === vNum);
          const enV = enVerses.find((v) => parseInt(v.verse) === vNum);
          merged.push({
            bookId,
            chapter,
            verse: vNum,
...
Respond ONLY with the JSON array. Do not include markdown code block formatting (such as \`\`\`json).`;
    const response = await ai.models.generateContent({
      ...
      config: {
        temperature: 0.1,
        // extremely low temperature for accurate recall
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        const seenVerses = /* @__PURE__ */ new Set();
        ...
        const formattedVerses = uniqueParsed.map((item) => ({
          bookId,
          chapter,
